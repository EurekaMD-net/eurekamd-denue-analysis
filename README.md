...
| **v0.2.10**    | Censo wider + locality    | View extension: `censo_municipios` 14→50 cols (religion/lengua/migración/assets/edu/salud) + new `censo_localidades` (193k filas). 3 endpoints `/localities-by-municipio` + `/locality-detail` + `/municipio-detail` | ✅ Done             | [v0.2-status.md](docs/v0.2-status.md)                               |
...
| Endpoints API funcionales         | 28 (+ `/health` = 29 routes) | 8 base + 20 `/analytics/*` cubriendo treemap nacional, sector-grade matrix, municipios×entidad, opportunity engine (vertical-agnostic AGEB/colonia), AGEB primitives (detail, by-municipio, opportunity), risk surface (SESNSP), mortality (EDR), state calibrators (ENIGH+ENOE), licensed pharmacies (COFEPRIS), sub-AGEB drilldown (manzanas, colonias), **airports-by-municipio** (SCT/AFAC traveler influx)                      |
