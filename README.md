...
v0.2.3 estructurado en 3 tiers tras investigación 2026-05-05 + corrección operador: **EDR/SINAIS mortalidad** = direct-join municipal (ship next, pendiente operator-supplied ZIP). **Datatur** = mixed (~50/70 destinos direct-join post-crosswalk; ~20 calibradores zonales). **ENOE + ENIGH** = calibradores estatales — parameter tables keyed por entidad que entran a endpoints existentes vía `LEFT JOIN` añadiendo columnas `_calibrated`. Patrón general: fuentes que no joinan a `cve_mun` no se descartan; condicionan / multiplican / contextualizan las filas municipales. Plan completo en [`docs/v0.2.3-plan.md`](docs/v0.2.3-plan.md).
...
| **v0.2.3**  | EDR + ENOE + ENIGH + Datatur | EDR mortalidad direct-join `cve_mun`; ENOE + ENIGH calibradores estatales (LEFT JOIN entidad); Datatur diferido por crosswalk | 📋 Plan listo       | [v0.2.3-plan.md](docs/v0.2.3-plan.md)                               |
